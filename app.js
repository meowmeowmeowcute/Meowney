import { calculateDebtRemaining, DIRECT_EXPENSE_PARENT_CATEGORY_NAME, MeowneyRepository } from './data-layer.js?v=59';
import { incomeExpenseAmount, parentCategoryBreakdown, runTransactionQuery, subcategorySummary } from './query-logic.js?v=59';
import { calculateClaimAmount, calculateExpression, updateExpression } from './calculator.js?v=59';
import { createBackup, exportTransactionsCsv, parseBackupText, planCsvImport } from './backup-format.js?v=59';
...
const compactAmount = (amount) => new Intl.NumberFormat('zh-TW', { maximumFractionDigits: 2, useGrouping: false }).format(amount);
...
function templateIsValid(template) {
  if (template.type === 'transfer') {
    return template.sourceAccountId !== template.targetAccountId
      && state.accounts.some((account) => account.id === template.sourceAccountId)
      && state.accounts.some((account) => account.id === template.targetAccountId);
  }
  if (!state.accounts.some((account) => account.id === template.accountId)) return false;
  if (template.type !== 'expense') return true;
  const parent = state.categories.find((item) => item.id === template.parentCategoryId);
  if (!parent) return false;
  if (parent.allowsDirectExpense && !template.subcategoryId) return true;
  return parent.children.some((child) => child.id === template.subcategoryId);
}

function templateSummary(template) {
  const typeLabel = template.type === 'expense' ? '支出' : template.type === 'income' ? '收入' : '轉帳';
  if (template.type === 'transfer') {
    const source = state.accounts.find((account) => account.id === template.sourceAccountId)?.name || '已刪除帳戶';
    const target = state.accounts.find((account) => account.id === template.targetAccountId)?.name || '已刪除帳戶';
    return `${typeLabel} · ${currency(template.amount)} · ${source} → ${target}`;
  }
  const account = state.accounts.find((item) => item.id === template.accountId)?.name || '已刪除帳戶';
  const category = template.type === 'expense'
...
async function useTemplate(id) {
  ...
  if (!template) return;
  if (!templateIsValid(template)) return showToast('這個範本使用的帳戶或類別已刪除，請到設定刪除或重新建立範本。');
  try {
    const input = { type: template.type, amount: template.amount, note: template.note, date: todayValue(), time: timeValue() };
    ...
    showToast(error.message || '套用範本時發生問題，請重試。');
  }
}
...
async function deleteTemplate(id) {
  const template = state.templates.find((item) => item.id === id);
  if (!template || !window.confirm(`刪除範本「${template.name}」？`)) return;
...
function templateInputFromForm() {
...
function templateValidationError() {
  const form = state.form;
  if (!['expense', 'income', 'transfer'].includes(form.type)) return { message: '目前交易類型不支援存為範本。' };
  if (!Number.isFinite(Number(form.amountText)) || Number(form.amountText) <= 0) return { message: '請先輸入大於 0 的金額。', selector: '.number-pad' };
  if (form.type === 'transfer') {
    if (!form.sourceAccountId || !form.targetAccountId) return { message: '請選擇來源與目的帳戶。', selector: '#transfer-account-section' };
    if (form.sourceAccountId === form.targetAccountId) return { message: '來源與目的帳戶不可相同。', selector: '#transfer-account-section' };
    return null;
  }
  if (!form.accountId) return { message: '請選擇帳戶。', selector: '#single-account-section' };
  if (form.type === 'expense' && !form.parentId) return { message: '請選擇母類別。', selector: '#category-section' };
  if (form.type === 'expense' && !selectedParent()?.allowsDirectExpense && !form.categoryId) return { message: '請選擇子類別。', selector: '#category-section' };
  return null;
}
...
async function saveCurrentFormAsTemplate() {
  ...
  const name = window.prompt('範本名稱（會顯示在主頁的常用範本列）', state.form.note || '');
  if (name === null) return;
  const trimmed = name.trim();
  if (!trimmed) return showToast('請輸入範本名稱。');
  const template = { id: crypto.randomUUID(), name: trimmed, ...templateInputFromForm() };
  state.templates = [...state.templates, template];
  await state.repository.setSetting('transaction-templates', state.templates);
  render();
  showToast(`已新增範本「${trimmed}」。`);
}
...
function formFromTransaction(transaction) {
  ...
  const singleReimbursementSource = transaction.isReimbursement === true && transaction.isBatchReimbursement !== true
    ? state.transactions.find((item) => item.id === transaction.reimbursementExpenseId)
    : null;
  return {
    id: transaction.id,
    ...
    note: singleReimbursementSource ? singleReimbursementSource.note || '' : transaction.note || '',
...
function renderSheet() {
  ...
  $('#debt-settlement-summary').innerHTML = debtSettlementReadOnly ? `
    ...
    <p><span>登記日期</span><strong>${escapeHTML(form.date.replaceAll('-', '/'))}</strong></p>` : '';
  ...
  const reimbursementQuickVisible = form.type === 'expense' && !reimbursementReadOnly && !batchReimbursementSource && !Boolean(form.debtDirection);
  $('#reimbursement-quick-toggle').hidden = !reimbursementQuickVisible;
  $('#reimbursement-quick-toggle').setAttribute('aria-pressed', String(form.reimbursementEnabled));
  $('#reimbursement-quick-toggle').classList.toggle('quick-reimbursement-toggle--active', form.reimbursementEnabled);
  $('#reimbursement-quick-toggle').innerHTML = form.reimbursementEnabled
    ? `<span>報銷</span><span>+${compactAmount(Number(form.reimbursementAmountText) || 0)}</span>`
    : '<span>報銷</span>';
  ...
  $('#note-input').disabled = batchReimbursementReadOnly || debtSettlementReadOnly || reimbursementReadOnly;
  $('#date-input').value = form.date;
  $('#date-quick-year').textContent = form.date.slice(0, 4);
  $('#date-quick-day').textContent = form.date.slice(5).replace('-', '/');
  $('#date-quick-field').classList.toggle('date-quick-field--disabled', reimbursementReadOnly || debtSettlementReadOnly);
  $('#date-input').disabled = reimbursementReadOnly || debtSettlementReadOnly;
  ...
  $('#save-as-template').hidden = !['expense', 'income', 'transfer'].includes(form.type) || reimbursementReadOnly || batchReimbursementSource || debtSettlementReadOnly || Boolean(form.debtDirection);
...
function validationError() {
  ...
  if (form.type === 'expense' && form.reimbursementEnabled && (!Number.isFinite(Number(form.reimbursementAmountText)) || Number(form.reimbursementAmountText) <= 0)) return { message: '請輸入大於 0 的報銷金額。', selector: '#reimbursement-quick-toggle', focusSelector: '#reimbursement-quick-toggle' };
  ...
  if (!form.date) return { message: '請選擇日期。', selector: '#date-quick-field', focusSelector: '#date-input' };
  if (form.type === 'transfer') {
...
async function saveTransaction() {
  ...
  try {
    ...
    else if (state.editingId && state.form.type === 'expense') await state.repository.updateExpenseWithReimbursement(state.editingId, input, { enabled: state.form.reimbursementEnabled, amount: Number(state.form.reimbursementAmountText), note: state.form.note.trim() });
    else if (state.editingId) await state.repository.updateTransaction(state.editingId, input);
    else if (state.form.type === 'expense' && state.form.reimbursementEnabled) await state.repository.createExpenseWithReimbursement(input, { amount: Number(state.form.reimbursementAmountText), note: state.form.note.trim() });
...
function render() { renderAccounts(); renderTemplates(); renderTransactions(); refreshQueryOptions(); renderQuery(); renderSettings(); }
...
function initialiseEvents() {
  ...
  $('#date-input').addEventListener('input', (event) => {
    clearFormValidation();
    state.form.date = event.target.value;
    $('#date-quick-year').textContent = state.form.date.slice(0, 4);
    $('#date-quick-day').textContent = state.form.date.slice(5).replace('-', '/');
  });
  ...
  $('#save-as-template').addEventListener('click', saveCurrentFormAsTemplate);
