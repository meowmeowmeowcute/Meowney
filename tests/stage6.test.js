...
export async function runStage6Tests() {
  ...
  await test('應用殼層完整快取所有執行期本機資源', async () => {
    ...
    for (const asset of ['./index.html', './styles.css?v=30', './app.js?v=30', './calculator.js?v=30', './data-layer.js?v=30', './query-logic.js?v=30', './backup-format.js?v=30', './manifest.webmanifest', './icons/meowney-192.png', './icons/meowney-512.png']) {
  ...
  await test('GitHub Pages 的 /Meowney/ 子路徑會正確解析 PWA 資源', async () => {
    ...
    assert(html.includes('href="manifest.webmanifest"') && html.includes('href="styles.css?v=30"') && html.includes('src="app.js?v=30"'), '頁面資源不是相對路徑或未使用一致的版本。');
    ...
    assert(worker.includes("meowney-app-shell-v30") && app.includes("./data-layer.js?v=30") && app.includes("./query-logic.js?v=30") && app.includes("./backup-format.js?v=30") && app.includes("./calculator.js?v=30"), '頁面、模組與離線快取版本不一致，可能混用新舊程式。');
